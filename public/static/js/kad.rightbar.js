...
function addFavorite(id) {
    var _url = "/Favorite/AddFavorite?wareCode=" + id;
...
function barLogin(callBack) {
    $("#login_boxs").show();
    barLoginCallBack = callBack;
}
...
function isFavored(ids) {
    ...
    if (!IsLogin())
        return;
    var _url = urlConfig.pc + "/Favorite/IsFavorite?productIds=" + ids;
    jQuery.ajax({
        type: "Get",
        url: _url,
        cache: false,
...
jQuery(document).ready(function () {
    setTimeout(barInit, 1500);
});

function barInit() {
    var _url = "/weight/get?id=nav_kadbar";
...
function bindEvent() {
    ...
    var $online_kefu_container = $(".online_kefu_container");
    ...
    if ($windowH < 800) { $(".navi_left").css("padding-top", 150); $(".sm_two_order").css("margin-top", 20); $(".online_kefu_container").css("top", "150px"); $(".isbd_mobile").css("top", "370px"); }
    ...
    $(".my_Info").bind('click', function (event) {
        ...
        if ($my_Info_isLogin.eq(_this).css('display') == "none") {
            $(this).css('background-color', '#0155af').siblings().css('background-color', '');
    ...
    $("#go_close_fixed").click(function () {
        popClose("showPrompt-fixed",true);
    });
    ...
    $commonWidth.hover(function () {
        ...
        switch (_this) {
            case 2:
                ...
                break;
            case 3:
                $(this).removeClass('my_Info_hover');
            ...
            case 6:
                ...
                break;
            case 7:
                $(this).removeClass('my-tsBox-hover');
                $(this).children(".order_current_container").stop().animate({ right: "66px", opacity: 0 }, 400).hide();
                $(this).children(".pointer").stop().animate({ right: "60px", opacity: 0 }, 400).hide();
                break;
            case 8:
    ...
    //在线客服切换效果
    setTimeout(function () { if ($online_kefu_container.is(':hidden')) { $online_kefu.click(); } }, 200);
    var onlineback = true;
    $online_kefu.bind('click', function () {
        if (!onlineback)
            return;
        onlineback = false;
		var productId = '',isShowSupport = '0';
        if (typeof (chatConfig) != "undefined") {
            productId = chatConfig.productId;
			isShowSupport = chatConfig.isShowSupport;
        };
		if(isShowSupport=='1'||productId==0){
        if ($online_kefu_container.is(':hidden')) {
            //$online_kefu_container.animate({ paddingRight: '-0px', width: "107px", left: "-366px", opacity: 'show' }, 1000, function () { onlineback = true; });
            if (navigator.userAgent.indexOf("MSIE")>0 && navigator.userAgent.indexOf("MSIE 7.0")>0) {
                $online_kefu_container.animate({ paddingRight: '-0px', width: "107px", left: "-109px", opacity: 'show' }, 1000, function () { onlineback = true; });
            }
            else{
                $online_kefu_container.animate({ paddingRight: '-0px', width: "107px", left: "-366px", opacity: 'show' }, 1000, function () { onlineback = true; });
            }
            $online_kefu.removeClass("online_hidden");
            $online_kefu.css("background-color", "#0155af");
        } else {
            
            if (navigator.userAgent.indexOf("MSIE")>0 && navigator.userAgent.indexOf("MSIE 7.0")>0) {
                $online_kefu_container.animate({ paddingRight: '-0px', width: "0px", left: '0', opacity: 'toggle' }, 1000, function () { onlineback = true; });
            }else{
                $online_kefu_container.animate({ paddingRight: '-0px', width: "0px", left: '-260px', opacity: 'toggle' }, 1000, function () { onlineback = true; });
            }
            //$online_kefu_container.animate({ paddingRight: '-0px', width: "0px", left: '-260px', opacity: 'toggle' }, 1000, function () { onlineback = true; });
            $online_kefu.addClass("online_hidden");
            $online_kefu.css("background-color", "");
        };
		}
    });
    //获取客服状态
    ...
    function GetChatStatus() {
        var preGroupId = 81,
			...
			isShowSupport = '0';
        if (typeof (chatConfig) != "undefined") {
            preGroupId = chatConfig.preGroupId;
            afterGroupId = chatConfig.afterGroupId;
            productId = chatConfig.productId;
			isShowSupport = chatConfig.isShowSupport;
        };
        var chatGroupId = "" + preGroupId + "," + afterGroupId + "";
        $(".kefu-li1 a").attr("href", "/Chat/ToChat?groupId=" + preGroupId + "&rukou=0&productId=" + productId);
        $(".kefu-li2 a").attr("href", "/Chat/ToChat?groupId=" + afterGroupId + "&rukou=1&productId=" + productId);
        if(isShowSupport!='1' && productId > 0)
			 $(".online_kefu").css("display","none")
		jQuery.ajax({
            url: window.urlConfig.multiDomain.pc() + "/Chat/GetGroupState?groupIds=" + chatGroupId + "",
            type: "Get",
            dataType: "jsonp",
            json: "callback",
            success: function (data) {
                for (var i = 0; i < data.length; i++) {
                    if (data[i].Status == 2) {
                        if (data[i].GroupId == afterGroupId) {
                            $(".fixed_navigation .online_kefu_container ul .kefu-li2").css("background-position", "12px -715px");
                        }
                        else {
                            $(".fixed_navigation .online_kefu_container ul .kefu-li1").css("background-position", "12px -715px");
                        }
                    }
                    else {
                        if (data[i].GroupId == afterGroupId) {
                            $(".fixed_navigation .online_kefu_container ul .kefu-li2").css("background-position", "12px -772px");
                        }
                        else {
                            $(".fixed_navigation .online_kefu_container ul .kefu-li1").css("background-position", "12px -772px");
                        }
                    }
                }
            }
    ...
    ///*咨询*/
    //$(".online_kefu_container ul li").click(function () {
    //    var chatUrl = $(this).find("a").attr("href");
    //    window.open(chatUrl);
    //});
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.Result) {
                ...
                $("#mycode").show();//我的二维码
                if (barLoginCallBack != null && (typeof barLoginCallBack) === "function") {
                    barLoginCallBack(1);
                    return;
                }
            }
...
//右边购物车
function barCartList(first) {
    if (first&&$("[name='kadCartNum']").length>0)
    {
        $(".kad-cartNums").text($("[name='kadCartNum']").eq(0).html());
        return;
    }
    var numbers = 0;
    var $windowH = $(window).height();
    $.ajax({
        ...
        success: function (data) {
            ...
            $(".kad-cartNums").text(data.TotalItemCount);
            $(".kad-cartNums").attr("name", "kadCartNum");
            var jn = data;
            if (!jn) { $("#myCarts").hide(); }
            ...
            //    return;
            //}
            if (jn && jn.IsOverLimit && jn.HadAdjusted) {
                alert(jn.OverLimitTips);
            }
            if (jn.SellerCart) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                if (data[i].IsRx == true) {
                    str += '<div class="pro_box clearfix"><div class="pic_show"><a target="_blank" href="/product/' + data[i].WareCode + '.shtml"><img alt="" src="' + data[i].Pic + '"></a></div><div class="changeNum"><a class="buy_againBtn" target="_blank" href="/product/' + data[i].WareCode + '.shtml">查看详情</a></div></div>';
                } else {
                    str += '<div class="pro_box clearfix"><div class="pic_show"><a target="_blank" href="/product/' + data[i].WareCode + '.shtml"><img alt="" src="' + data[i].Pic + '"></a></div><div class="changeNum"><a class="buy_againBtn" href="javascript:;" onclick="rightAddCart_new(1,\'' + data[i].WareCode + '\',0,\'' + data[i].SellerCode + '\')">加入购物车</a></div></div>';
                }
                str += "</li>";
            }
            str += "</ul>";
...
var kadbar=(function () {
    ...
    _kadbar.prototype.addCart=function(){
        autoBar();//加入购物车时,右侧导航自动展开
