...
function GetTopCartList() {
    var numbers = 0;  
    jQuery.ajax({
        url: window.urlConfig.multiDomain.pc() + "/cart/GetCartCount",
        type: "Get", cache: true, dataType: "jsonp", jsonp: "callback",
        success: function (data) {           
 
                var str ="<a href=\""+cartDomain+"/cart/index\">购物车"+ (data.TotalItemCount||0) +"件</a>"    
...
function topLogin() {
    location.href = urlConfig.user + '/Login?ReturnUrl=' + location.href;
};
function topReg() {
    location.href = urlConfig.user + '/Register/Mobile?ReturnUrl=' + location.href;
};